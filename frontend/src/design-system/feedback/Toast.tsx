  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        {title && <div style={{ fontFamily: "var(--font-ui)", fontWeight: 700, fontSize: "var(--text-md)", color: "var(--text-strong)" }}>{title}</div>}
        {message && <div style={{ fontFamily: "var(--font-body)", fontWeight: 600, fontSize: "var(--text-sm)", color: "var(--text-muted)", marginTop: 2 }}>{message}</div>}
      </div>
      {onClose && (
        <button type="button" onClick={onClose} aria-label="Dismiss" style={{
          ...
          background: "transparent", border: "none", color: "var(--text-muted)", flexShrink: 0,
        }}>
          <Icon name="x" size={16} />
        </button>
      )}
